...
export default function SellerStats() {
  ...
  const [finStats, setFinStats] = useState<SellerStatsData | null>(null);
  ...
  useEffect(() => {
    async function fetchStats() {
      try {
        ...
        const res = await fetch(`https://seng2021-notuna-order.vercel.app/shop/seller/get-seller-stats?s_id=${sellerId}`);
  ...
  return (
    <div className="container py-4 bg-light p-4 rounded mt-5 shadow-sm">
      <h1 className="h4 mb-4 fw-bold text-dark text-center">Seller Stats</h1>
      <div className="mb-4 fw-bold text-dark text-center">Total Orders: {stats?.totalOrders ?? 'Loading...'}</div>
      <h2 className="h5 mt-5 text-center">Revenue by Order Status</h2>
