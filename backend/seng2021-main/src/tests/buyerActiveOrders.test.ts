...
describe('GET /shop/buyer/:buyerId/active-orders', () => {
  ...
  test('should return 400 if email is not provided', async () => {
    const response = await request(app).get(`/shop/buyer/${buyerId}/active-orders`);
  ...
  test('should return 404 if email does not exist in DB', async () => {
    ...
    const response = await request(app)
      .get(`/shop/buyer/${buyerId}/active-orders`)
  ...
  test('should return 200 and list of orders excluding ORDER_REGISTERED', async () => {
    ...
    (buyerService.getOrdersByBuyerEmail as jest.Mock).mockResolvedValueOnce([
      ...
      { orderId: 3, status: 'ORDER_REGISTERED' } // This should not appear
    ]);
    ...
    expect(response.body.orders).toHaveLength(3); // Mock still returns 3
    // Real query in DB should filter ORDER_REGISTERED, but since mock returns 3, we check:
