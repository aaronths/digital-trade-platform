...
export const hasOrderStatus = async (orderId: number, status: string): Promise<boolean> => {
...
export const getOrderDetails = async (orderId: number): Promise<OrderDetails> => {
  ...
  : '';
        ...
        throw new Error(`Error fetching order details: ${(error as Error).message}`);
    }
};
...
export async function createSellerResponse(orderId: number, newResponse: string) {
        ...
        const updateResponseQuery = 'UPDATE Orders SET response = $1 WHERE order_id = $2';
        await pool.query(updateResponseQuery, [updatedResponse, orderId]);

        // Update the order status
        const updateStatusQuery = 'UPDATE Orders SET o_status = $1 WHERE order_id = $2';
...
interface orderCreateObject {
    ...
    Details: string,
    OrderStatus: string,
    Buyer: {name : string, tax: string, phone: string, email: string, address: string},
    Seller: {name : string, tax: string, phone: string, email: string, address: string},
...
//helper function for generateInvoice
async function generateInvoiceAPI(orderId : number, xmlInvoice : string) : Promise<string> {
    try {
        const response = await fetch('http://guard-ubl-api.us-east-1.elasticbeanstalk.com/api/invoices/generate', {
        method: 'POST',
        headers: {
...
export async function generateInvoice(orderId: number, sellerId: number) : Promise<string> {
    if (!orderId || isNaN(orderId)) {
        throw new Error('OrderId is empty or invalid');
    }
    ...
    const isOrderValidQuery = 'SELECT seller, invoice_id, o_status FROM Orders WHERE order_id = $1';
...
export async function getSellerRegisteredOrders(sellerId: number) {
    ...
    const checkQuery = 'SELECT order_id FROM Orders WHERE seller = $1 AND o_status = $2';
    const checkResult = await pool.query(checkQuery, [sellerId, status]);
    const orderIds = checkResult.rows.map(row => row.order_id);
    return orderIds;
}

export async function saveInvoice(orderId: number, sellerId: number, invoiceId: string) {
    if (!orderId || isNaN(orderId)) {
        throw new Error('OrderId is empty or invalid');
    }
    ...
    const SaveInvoiceQuery = 'UPDATE Orders SET invoice_id = $1 WHERE order_id = $2';
    await pool.query(SaveInvoiceQuery, [invoiceId, orderId]);
};


export async function viewInvoices(buyerOrSellerId: number, BuyerOrSeller : string) {
    ...
    if (BuyerOrSeller === "seller") {
        InvoiceListQuery = 
        `SELECT o.order_id, o.order_date, b.b_name, o.o_status, p.p2_name, o.price, o.invoice_id, o.seller FROM Orders o 
        JOIN buyers b ON o.buyer = b.b_id
        JOIN productsv2 p ON o.product = p.p2_id
        ...
        InvoiceListQuery = 
        `SELECT o.order_id, o.order_date, s.s_name, o.o_status, p.p2_name, o.price, o.invoice_id, o.buyer FROM Orders o 
        JOIN sellers s ON o.seller = s.s_id
        JOIN productsv2 p ON o.product = p.p2_id
...
*/
  ...
  async function callNewDespatchAPI(despatchData: DespatchRequestNew): Promise<object> {
  ...
  function transformToNewDespatchFormat(order: DespatchOrder): DespatchRequestNew {
    ...
    console.log('Contact Info - Phone:', order.buyer.phone, 'Email:', order.buyer.email);
  ...
  export async function saveDespatch(orderId: number, sellerId: number, despatchId: string) {
    if (!orderId || isNaN(orderId)) {
        throw new Error('OrderId is empty or invalid');
    }
    ...
    const isOrderValidQuery = 'SELECT seller, despatch_id, o_status FROM Orders WHERE order_id = $1';
    const isOrderValidRes = await pool.query(isOrderValidQuery, [orderId]);
    if (isOrderValidRes.rowCount == 0) {
        throw new Error('OrderId does not exist');
    }
    ...
    const SaveDespatchQuery = 'UPDATE Orders SET despatch_id = $1 WHERE order_id = $2';
    await pool.query(SaveDespatchQuery, [despatchId, orderId]);
};

export async function viewDespatches(buyerOrSellerId: number, BuyerOrSeller : string) {
    ...
    if (BuyerOrSeller === "seller") {
        ...
        DespatchListQuery = 
        `SELECT o.order_id, o.order_date, b.b_name, o.o_status, p.p2_name, o.price, o.despatch_id, o.seller FROM Orders o 
        JOIN Productsv2 p ON o.product = p.p2_id
        JOIN buyers b ON o.buyer = b.b_id
        ...
        DespatchListQuery = 
        `SELECT o.order_id, o.order_date, s.s_name, o.o_status, p.p2_name, o.price, o.despatch_id, o.buyer FROM Orders o 
        JOIN Productsv2 p ON o.product = p.p2_id
        JOIN sellers s ON o.seller = s.s_id
    ...
    const DespatchListRes = await pool.query(DespatchListQuery, [buyerOrSellerId]);
